...
\item if tasks are independent, then the user mays introduce a synchronization point using the \texttt{\#pragma kaapi sync} directive to force all tasks created before the synchronization point to be completed at the end of the synchronization point. In the previous example, this directive must be placed between line 13 and line 14.
\end{itemize}

The same effect could be obtained by defining a sequence of two parallel region. 
%Note that at the end of a parallel region there is an implicit synchronization point: to enforce order in the previous example, the user may also have created tasks inside 2 parallel regions. 
...
The example of figure~\ref{ex:simpleWR} creates two tasks in a parallel region defined at line 18. The tasks are created at line 20 and 21.
The first task takes 3 parameters: the first one is the size of the buffer parameter, the second parameter. A third parameter points to the message to copy into the buffer. The respective access modes for these parameters are: 
\begin{itemize}
\item \texttt{value} for the size: this access mode is for data passed by \textit{value}, the function call receive a copy to the value that can be modified in the function body, but modification is not visible outside the function body execution.
\item \texttt{write} for the buffer: in this example, the function writes characters into the buffer. All update to the buffer is visible outside the function. Moreover, any future read access to the same buffer introduces a read-after-write dependency.
\item \texttt{read} for the messsage. 
\end{itemize}
They are defined at line 3.
...
The same effective parameter \texttt{buffer}, declared at line 17, is passed between the two tasks at line 20 and 21. Because of the previous declaration of \texttt{write} access in the first task and \texttt{read} access in the second task, it induces a \textbf{true} data flow dependency between the both tasks.
...
Due to access mode, this two tasks have a true dependency. Like other examples, these tasks are created and executed inside a parallel region.
...
Task creation is a non blocking call, thus at line 13 a synchronization point is added in order to wait for the completion of all the previous created tasks in the same parallel region, \textit{i.e.} completion of the two previously created tasks.
...
No more synchronization is required, but the user must take care of the scope of the variables  passed as effective  parameters to the tasks.
...
At runtime, the shared memory writers are executed sequentially.
...
A \kaapi program is composed by C or C++ code and some annotations specified by a programmer to indicate what function to use to create tasks when they are called.
A \kaapi program begins by executing the C/C++ main entry point of the processus. Then, each time a call to an annotated function is encountered, a task is created and the control flow continue its execution until it reaches a synchronization point.\\
 
A task is a function call without side effect: a function that should return no value except through the shared memory and the list of its effective parameters. It is the responsibility of the programmer to annotate functions for which calls become tasks by using the \texttt{\#pragma kaapi task} directive. The parallelism in \kaapi is explicit (task annotation) while the detection of synchronizations is implicit: the dependencies between tasks and the memory transfers are automatically managed by the runtime.\\
...
Tasks share data if they have access to the same memory region. A memory region is defined as a set of addresses in the process virtual address space. This set has a shape: a multi-dimensional array.

The user is responsible to indicate the memory parameter access modes: the main access modes are \textit{read}, \textit{write} or \textit{exclusive}.\\

One of the main characteristic to the \kaapi programming model is that it preserves the sequential semantics of the program: the result of the parallel execution remains the same as the sequential execution of the program compiler without the \kaapi compiler.
...
\begin{itemize}
\item The declaration of a task is a function declaration annotation by \texttt{\#pragma kaapi task} with named formal parameter. 
...
At line 1, we declear the parameter \texttt{n} is to be passed by value; the parameter \texttt{array} references a 1-dimensional array of size \texttt{n} that is to be read; and the parameter \texttt{result} is written by the task to store a result.
Typically, this result is read by a task that takes it with a \texttt{read} access mode, such as the task declared at line 4.

They are two kinds of access mode for parameter:
\begin{itemize}
...
The creation of a task is simply represented by the call to the function annotated to be a task. The \kaapi replaces all function calls to tasks by the creation of a task structures that captures the effective parameters.

A major difference with the sequential call to function is the \kaapi task creation is a non blocking operation: the callee does not wait for the task completion before the execution continues. At runtime, the thread of control that generates tasks that will be executed in concurrence depending of the available resources. The execution flow of C/C++ code and the execution flow of tasks are not synchronized except implicitly on certain point, or on request to the programmer to synchronize them (see section~\ref{sec:synchro}).
...
Parallel regions may be embedded: the parallel regions share the same set of threads as concurrent execution.
...
The scope of a parallel region is the next instruction or the next basic block (between '{' and '}') if the directive is placed just before. For instance in figure~\ref{fig:accumparallelregion}, the declaration of the parallel region at line~6 only applies to the statement at line 7 that creates a task.

\begin{verbatim}
#pragma kaapi parallel 
\end{verbatim}

The runtime select the number of threads using 1/ \kaapi environment variable \texttt{KAAPI\_CPUCOUNT} and \texttt{KAAPI\_CPUSET}, that defines both the number and a set of cores to used; 2/ else  \kaapi creates one thread per physical core of the machine.
...
In special context, there are implicit parallel regions. Currently the task definition (function body of a task definition) is a parallel region. Recursive calls to a task definition function creates tasks. See section~\ref{sec:recursive} about parallel recursive programming with \kaapi.
...
Several access modes have been defined since the first vesion Athapascan~\cite{Athapascan98}: \textbf{read}, \textbf{write}, \textbf{readwrite} or \textbf{exclusive} and \textbf{cumulative write}. Each of these access mode limits the kind of operation a task can perform on a memory region. For instance, a task that declares an access mode read to one of its effective parameters cannot write value. These access modes have either a direct correspondance in StarSs\footnote{Called direction as in CORBA} or PLASMA; or are similar (reduction in StarSs).
