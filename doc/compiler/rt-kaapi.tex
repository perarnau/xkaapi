...
%%
\RRtitle{Le modèle de programmation \kaapi \\et manuel d'utilisation}
%% English title
...
Other packaging (Debian or RPM packages) is under development. Please visit the \kaapi web site for an up-to-date information:
\begin{center}
\url{http://kaapi.gforge.inria.fr}
\end{center}
...
./configure  --prefix=$HOME/install --enable-mode=release --with-hwloc=/usr/local/hwloc --with-perfcounter --with-boost=/usr/local/boost --with-rose=/Users/thierry/Developpment/KAAPI/ROSE/install --enable-kacc --disable-api-atha --disable-api-kastl
\end{verbatim}
...
\begin{itemize}
\item the directive enforces that all previously created tasks in the same function body\footnote{More formally, in the same activation frame as the function body execution.} be completed before proceeding with the next instruction at line 14 to sum the sub results.
\item the directive does \textbf{not} wait for the completion of tasks created in the outter function calls.
\end{itemize}
...
A task is a function call: a function that should return no value except through the shared memory and the list of its effective parameters. It is to the responsibility of the programmer to annotate functions which calls become tasks, by using the \texttt{\#pragma kaapi task} directive. 
...
Once defined, any calls to annotated functions are compiled by task creations. The runtime runs every task that is ready by performing the instructions of the function body. 
...
\label{fig:exdecldef}
\end{figure}
In figure \ref{fig:exdecldef}, the line 1 and 2 correspond to the function task declaration. The line 3 through 4 to the function task definition.
...
The runtime selects the number of threads using 1/ \kaapi environment variables \texttt{KAAPI\_CPUCOUNT} and \texttt{KAAPI\_CPUSET}, that define, respectively, the number of cores and the specific cores to used; else 2/ \kaapi creates one thread per physical core of the machine.
...
Several access modes\footnote{These access modes have either a direct correspondence in SMPSs/StarSs, called direction as in CORBA. Same access modes exist in Quark, the PLASMA abstract layer for task creations.}
 have been defined since the first version of Athapascan~\cite{Athapascan98}: \textbf{read}, \textbf{write}, \textbf{readwrite} or \textbf{exclusive} and \textbf{cumulative write} (also called \textbf{reduction}). Each of those access modes limits the kind of operation a task can perform on a memory region. For instance, a task that declares an read access mode for one of its effective parameters cannot write.
...
The \texttt{<list declaration>} defines a list of memory regions touched by the task. 
...
To split data structures for parallel sub-computations, it is necessary to handle sub sets of 2-D arrays.

\kaapi  follows the design of BLAS or LAPACK libraries: the matrix is assumed to be allocated into a contiguous array and the user must specify the \textit{leading dimension} (\textit{ld} in figure~\ref{fig:matrix}), which is the size necessary\footnote{The size is expressed with respect to the \textit{element type}.}  to pass from one row to the next one with the C storage (from one column to the next column with Fortran convention).
...
\item The dimension must follow the sub set of the C grammar for expression. 
\end{itemize}
...
Any C or C++ type can by defined to be passed by value.
The syntax is the following:
\begin{verbatim}
#pragma kaapi task value( <list declaration> )
\end{verbatim}
The rule \texttt{<list declaration>} is given in the appendix~\ref{sec:grammar} and corresponds to a list separated by '\verb+,+' of memory regions.
...
\end{minipage}
\end{center}
\caption{Section of variable to enforce synchronization.}\label{ex:fibonacci}
\end{figure}
...
\kaapi allows for recursive task creation. Several examples have been presented in the section~\ref{sec:helloworld}.
...
All data passed by reference to tasks must be valid until the task completions.
For instance, because a task creation is a non-blocking call, the example of figure~\ref{ex:caplante} is incorrect: the automatic variable \texttt{r} will be suppressed before the execution of the task \texttt{g}.
\begin{figure}[ht]
\begin{center}
\begin{minipage}{0.99\linewidth}
\begin{footnotesize}
...
\begin{itemize}
\item Add a synchronization point between line 4 and 5. It enforces the execution to wait for the completion of \texttt{g} (the 2 left solutions in figure~\ref{ex:caplanteplus}).
\item Allocate the data in the same scope as the tasks. In that case, the data is allocated with a scope wide enough to execute the task \texttt{g} (right solution in figure~\ref{ex:caplanteplus}).
\end{itemize}
...
The \kaapi name of this directive is analog to the \texttt{alloca} function of the C library stdlib.  With sequential C or C++ program, the developper may allocate data in the stack using the \texttt{alloca} function. In that case, the allocation scope is the same as the activation frame where the call to \texttt{alloca} occured.
...
This example shows that one of the main difference between StartSs programming model is its ability to view sub-matrix of matrix without requiring to reallocate the user data structure.\\
...
\begin{description}
\item [-\,-keep]: keep the intermediate files. For each file, one intermediate files are generated. It is prefixed by \texttt{kaapi-} and is the translation of the file with replacement of function calls by task creations. 
\end{description}
...
A parallel  SMPSs program running on a multicore is composed of several threads sharing the same address space. The threads execute tasks which may share values in the address space, that we call thread shared memory.
...
All tasks are asynchronously as in \kaapi.
