...
GNU-Linux with x86\_64 architecture and MacOSX/Intel processor.
...
Once defined, any calls to annotated functions are compiled FIXME? by task creations. The runtime runs every task that is ready by performing the instructions of the function body. 
...
Several access modes\footnote{These access modes have either a direct correspondance in SMPSs/StarSs, called direction as in CORBA, as well as in PLASMA. FIXME: either -> or ?}
...
To split data structures to execute in parallel sub-computations, it is necessary to handle sub sets of 2-D arrays.

\kaapi  follows the design of BLAS or LAPACK libraries: the matrix is assumed to be allocated into a contiguous array and the user should specify the \textit{leading dimension} (\textit{ld} in figure~\ref{fig:matrix}), which is the size necessary\footnote{The size is expressed with respect to the \textit{element type}.}  to pass from one row to the next row with the C storage (from one column to the next column with Fortran convention).
...
\item [Write after Read.] This false dependency is an anti-dependency. Variable renaming permits to suppress them.
\end{description}
...
\kaapi allows recursive task creations. Several examples have been presented in the section~\ref{sec:helloworld}.
%This is illustrated in figure~\ref{fig:exaccu} (page~\pageref{fig:exaccu}).
In the accumulate code of figure~\ref{fig:exaccu} (page~\pageref{fig:exaccu}) recursively splits the computation until a grain of 2. Each created sub-tasks works on a disjoint part of  the initial input array. 

This example is simple: no temporary data are required at each level of recursion. This not the case with the recursive computation of the Fibonacci number of figure~\ref{ex:fibonacci} page~\pageref{ex:fibonacci}.
...
\begin{description}
\item [-\,-keep]: keep the intermediate files. For each file, 2 intermediate files are generated. The first one is prefixed by \texttt{kaapi-} and is the translation of the file with replacement of function calls by task creations. FIXME: the second is ?
\end{description}
...
\begin{description}
\item [KAAPI\_CPUSET]: if defined, and if the operating system supports it, then this variable specifies the set of cores to use in \kaapi programs. If not defined, the operating system decides to schedule \kaapi threads onto FIXME: all cores.
