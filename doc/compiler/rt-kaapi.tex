...
\RRabstract{During the past years, dynamically scheduled data flow graph for tiled algorithms in linear algebra appears as a promising way to reach the portability of performance onto several multicore architectures.
The idea was to describe these algorithms into an abstract data flow graph representation that only encodes tasks and their dependencies independently of the scheduling algorithm, which folds up tasks onto the cores of the target architecture. 
Reported experimental timings show good performances for linear algebra algorithms where tasks are generated only from nested loops. Nevertheless, all recently proposed frameworks do not consider program where tasks with data flow dependencies are generated recursively. 

In this report, we present the \kaapi's programming model. A parallel \kaapi program is a C or C++ sequential program with code annotation using \texttt{\#pragma} compiler directives. A specific source to source compiler translates \kaapi directives to runtime calls.
}
...
Several research projects~\cite{Gunnels:2001:FFL,plasma-analysis,PerezBL08} have investigated the use of data flow graphs as an intermediate representation of the computation of LAPACK's algebra algorithms~\cite{plasma-lapack}: the main reason was that the portability of performance in LAPACK is of the responsibility of a set of basic linear algebra subprograms (BLAS), which exhibit only low level parallelism that is not enough on multicores.

Without taking care of considerations about  tile algorithms to reach performances, three majors points have been identified as important to perform efficiently tile algorithms~\cite{Gunnels:2001:FFL,plasma-analysis,plasma-lapack}: 1/ fine granularity to reach high level of parallelism; 2/ asynchronous execution to prevent barrier of synchronization; and 3/ a dynamic data driven scheduler to ensure execution of task as soon as all their input data are produced.
...
Nevertheless, most of the recently proposed frameworks that share the data flow graph as a central representation to schedule dynamically tasks according to their dependencies, are not able to perform programs with recursive tasks creations.  
...
\kaapi programming model comes from Athapascan~\cite{Athapascan98} which, itself, was inspired by Jade~\cite{Jade:1998} and Cilk~\cite{Cilk5:1998}. Because \kaapi programming model and  StarSs~\cite{Badia:2009} was very closed, we also accept the execution of  StarSs program. Descriptions of \kaapi may be found in following publications~\cite{Kaapi_2007,Traore_2008-deque_free, CCK_MCO08,KAAPI_TIC_2009}.
...
In this report, only the programming model based on code annotation with \texttt{pragma} compiler directive is presented.
The runtime library comes also with a full set of complementary programming interfaces: a C, C++ and STL like interface. The C++ and STL interfaces, at higher level than the C interface,  may be directly used for developing parallel programs or libraries.
...
If you whish to contact the XKaapi team web site at:
\begin{center}
...
\end{itemize}
The \kaapi libraries and runtime are available at \url{http://kaapi.gforge.inria.fr} (tarball) or as a debian package. \kaapi libraries and runtimes are distributed with a CeCILL-C license\footnote{\url{http://www.cecill.info/index.en.html} for english version}:
\begin{center}
...
\end{itemize}
Precise definition of the procedure to install rose and EDG frontend is described in the ROSE documents.
...
system, in which case the log will give you the names of the software to install.
...
Executables linked with KaCC does not require to add this directory in your \verb+LD_LIBRARY_PATH+ (under linux) or \verb+DYLD_LIBRARY_PATH+ (under Mac OS X).
...
Without brackets it only concern the following statement.
%It could also be defined before any instruction and it will end after the instruction.

The \texttt{\#pragma kaapi parallel} defines a region where concurrent executions may occur. The number of threads used to execute tasks inside a parallel region may varies during the execution. 
...
The three ways \kaapi offers to enforce an execution order is:
\begin{itemize}
\item Tasks have declared special access modes to their parameters (read, write, exclusive), which introduce read-after-write data flow dependencies between tasks: this are the only dependencies that \kaapi respect at runtime. This is the subject of the next section.

\item If tasks are independent, then the user mays introduce a synchronization point using the \texttt{\#pragma kaapi sync} directive to force all tasks created in the same function body before the synchronization point to be completed at the end of the synchronization point. In the previous example, this directive must be placed between line 13 and line 14:
\begin{footnotesize}
\lstset{numbers=left, numberstyle=\tiny, stepnumber=1, numbersep=5pt}
\lstset{commentstyle=\color{blue}}
\lstset{language=C}
...
\item The user mays also defined a sequence of parallel region in order to take benefit of the implicit synchronization at the end of each parallel region.
The code of figure~\ref{ex:helloworld2} would become:
\begin{footnotesize}
\lstset{numbers=left, numberstyle=\tiny, stepnumber=1, numbersep=5pt}
\lstset{commentstyle=\color{blue}}
\lstset{language=C}
...
The first task takes 3 parameters: the first one is the size of the buffer parameter, the second parameter. A third parameter points to the message to copy into the buffer. The respective access modes for these parameters, defined at line 3, are:
\begin{itemize}
\item \textbf{by value access mode}, for the parameter \texttt{size} defined by \texttt{'value(size)'}. 
This access mode is like the pass-by-value rule of C/C++ function call: The callee copies the effective parameter\footnote{The expression passed in the function call.} to the formal parameter\footnote{The name of the variable in the function definition}. The callee receives a copy of the value that can be modified. Modifications are not viewed by the caller.
...
This parameter is the C string reads by the task and copied into the parameter \texttt{buffer}.
...
Due to access modes, this two tasks have a true dependency. Like the other examples, these tasks are created and executed inside a parallel region defined in the main function, at line 
...
\begin{itemize}
\item the directive enforces that all previously created tasks in the same function body\footnote{More formally, in the same activation frame of the function body execution.} be completed before to proceed the next instruction at line 14 to sum the sub results.
\item the directive does \textbf{not} wait completion of tasks created on the outter function calls.
\end{itemize}
...
In place of adding a synchronization point, we create, at line 18, a task \texttt{sum}, with true data flow dependencies with the previous tasks (on variables \texttt{r1} and \texttt{r2}). This \texttt{sum} task do the summation (task sum definition at line 1) of the two sub-results computed by the recursive tasks created at line 16 and 17.
...
Because the tasks created at lines 16, 17 and 18 are non blocking call, the user must takes care of the scope of the variables  passed to the tasks. In the previous example, because \texttt{r1} and \texttt{r2} are allocated into the C stack of the function \texttt{fibonacci}, the user must specify at line 14 the directive  \texttt{\#pragma kaapi data alloca} to annotate explicitly these data: Then the compiler will allocate them such that their scope is enough for the execution of the task.
...
The second form is to specify an interval between two addresses (as used in most of STL algorithms). The syntax for interval is \texttt{[a:b]}, where \texttt{a} and \texttt{b} are identifiers of formal parameters.
\begin{verbatim}
...
\item The dimension must follows the sub set of the C grammar for expression. 
\end{itemize}
...
A task that require an accumulative semantic with respect to one of its formal parameters, must declare it with the \texttt{reduction} clause in the \texttt{\#pragma kaapi task}. 
...
The \verb!function-name! must correspond to the name of C/C++ function that must satisfy one of the following signature:
...
In the accumulate code of figure~\ref{fig:exaccu} recursively splits the computation until a grain of 2. Each created sub-tasks works on a disjoint parts of  the initial input array. 
