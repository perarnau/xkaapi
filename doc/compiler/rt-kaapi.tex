...
Due to access modes, this two tasks have a true dependency. 
%Like the other examples, these tasks are created and executed inside a parallel region defined in the main function, at line FIXME
...
\begin{itemize}
\item the directive enforces that all previously created tasks in the same function body\footnote{More formally, in the same activation frame of the function body execution.} are completed before proceeding with the next instruction, at line 14, to sum the sub results.
...
The execution of a \kaapi program generates a sequence of tasks that access to data into a shared memory. From this sequence, the runtime is able to extract independent tasks in order to dispatch them amongst the idle cores.
...
The user is responsible for indicating the task memory access modes: the main access modes are \textit{read}, \textit{write}, \textit{reduction} or \textit{exclusive}.\\
...
This section presents how to annotate a function declaration in such a way that ANY call to this function are translated to creation of tasks. This is called \textit{function task} annotation.
...
Once defined, all the annotated function calls are replaced by task creations. The runtime runs each ready task by performing the instructions of the function body. 
...
As for functions, a function task declaration represents the signature of a function. The task body is the body of the annotated function. This is the instructions performed at runtime to execute the task.
...
A major difference with the sequential call to function is that the \kaapi task creation is a non blocking operation: the caller does not wait for the task completion. 
...
The runtime selects the number of threads using
\begin{enumerate}
\item \kaapi environment variables \texttt{KAAPI\_CPUCOUNT} and \texttt{KAAPI\_CPUSET}, that define, respectively, the number of cores and the specific cores to used;\item Otherwise  \kaapi creates one thread per physical core of the machine.
\end{enumerate}
See appendix~\ref{sec:compilo} for a detailed description of the environment variables used by \kaapi.
...
The access mode is defined for a memory region, \textit{i.e.} a set of addresses in the virtual address space of the process.
...
A memory region is described by:\
\begin{itemize} 
...
\begin{enumerate}
\item \texttt{n}: this is the memory region starting at address of the variable \texttt{n} whose size is \texttt{sizeof(int)} bytes.
\item \texttt{array[n]}: this a 1-dimensional array starting at address \texttt{array} and ending at address \texttt{array+n} (exclusive).
\item \texttt{result}: its memory region corresponds to the the storage of the variable of type \texttt{int} pointed by the value of \texttt{result}.
\end{enumerate}
...
The second form is to specify an interval between two addresses (as used in most of STL algorithms). The syntax for interval is \texttt{[a:b]}, where \texttt{a} and \texttt{b} are formal parameters identifiers.
\begin{verbatim}
...
\begin{description}
\item [write:] the task can write any of the location of the declared memory region. Any read instruction is guarantee to return the value produced by the task. A read instruction to a location no written by the task returns an undefined value.
\item [read]: the task that reads a location in the memory region declared \texttt{read} accesses the value produced by the previous, in the lexicographic order of tasks in the program, task that has required a write or read-write or cumulative access. If no such task exists, then the value is the value given during initialization.

The task cannot write location of the memory region.
\item [readwrite:] the task can read or write any location of the memory region. As for \texttt{read} access mode, a read instruction returns the value previously written by the task or the value produced by a previous task or at initialization.
\item [reduction:] this is the purpose of the next section.
\end{description}

%%%%
\subsubsection{Cumulative access mode}\label{sec:cw}
In \kaapi, it is possible to reduce a variable with an associative operator. The commutativity is not required.
...
The reduction clause specifies the operator to use during concurrent reduction and a list of formal parameters the clause is applied to.

Two tasks having a reduction access mode onto the same memory region are concurrent, for instance the two tasks created at line 12 and 13 in listing of figure~\ref{fig:exaccu}.
\begin{figure}[ht]
\begin{center}
\begin{minipage}[t]{0.99\linewidth}
\begin{footnotesize}
...
\label{fig:exaccu}
\end{figure}
The reduction operator defined in the \texttt{reduction} clause is used to merge result produced by a theft task.
...
The \verb!function-name! must correspond to the name of C/C++ function. The function must comply with one of the following signature:
...
The identity function is used to initialize a value of a temporary variable to be the neutral of the reduction operator. The identity function signature must be one of the following:
\begin{verbatim}
  // For  C++
...
Note that the \kaapi runtime used a lazy approach to remove false dependencies: if it is necessary to execute concurrently two tasks with false dependencies, then the runtime renames the memory region. Otherwise the execution is sequential, using the task creation order.

In both cases, the result of the execution remains the same whatever the execution order between tasks.
...
Because task creations are non blocking instructions, it may be
necessary to explicitly synchronize the control flow of the thread that creates tasks with the task executions.
...
For instance, the synchronization point at line 13 of the recursive computation of Fibonacci number in listing~\ref{ex:fibonacci} page~\pageref{ex:fibonacci} enforces completion of only the two tasks created at lines 11 and 12.


\subsubsection{\#pragma kaapi sync(<list\_declaration>)}

This directive forces to wait for the completion of all the last created tasks, in the same task body activation, which writes the memory region  described by a the list of declarations.
\begin{figure}[ht]
\begin{center}
\begin{minipage}{0.99\linewidth}
\begin{footnotesize}
...
\begin{itemize}
\item Add a synchronization point between line 4 and 5. It enforces the execution waits for the completion of \texttt{g} (the 2 left solutions in figure~\ref{ex:caplanteplus}).
...
With \texttt{\#pragma kaapi alloca}, the user can allocate data within the same scope of the created tasks.
...
This example shows one of the main difference between the StarSS programming model is its inability to view sub matrix without reallocating the user's data structure.\\
...
If the reduction operator for a \verb+reduction+ access mode is left unspecified, then the default operator is \verb'+' for all the scalar type (integral value and floating point value). 
For other instance of type or class that does not have the \verb'+' operator defined, then this is an compiler error. The default identity operator is:
\begin{itemize}
\item for scalar value (integral value and floating value) to set the value '0' which is the neutral element with respect to the associative default \verb'+' operator,
\item For other type or class which have a defined \verb'+' operator, then the neutral is assumed to be set by calling the default constructor.
\end{itemize}

\subsubsection{Note on range declaration}

In the range declaration rule, it is possible for the programmer to define the set of memory address that the task accesses. 
The set of memory addresses may have any shape. This set is used by the runtime to detect dependencies between tasks. The implementation has currently 2 limits:
\begin{enumerate}
...
It is a compiler error to pass a non automatic name (\textit{e.g.} global variable) or to declare the pragma after the variable definition.
...
\begin{enumerate}
\item Each \kaapi directive  that begins \texttt{\#pragma kaapi} is interpreted and the code is rewritten to call functions of the \kaapi library. Invalid use of \kaapi directive and some warning are outputed.
...
\begin{description}
\item [-\,-keep]: keep the intermediate files. For each source file, an intermediate file is generated. It contains the generated calls to \kaapi. Its name follows the convention: if the source name is \texttt{file.c}, the intermediate name is \texttt{file\_\_kacc.c}.
\end{description}


\subsection{Backend compiler selection}
The backend compiler is the default C or C++ compiler defined during the installation step (see options with \texttt{configure --help}). It can be changed at configuration by setting the environment variables \texttt{KACC\_CC} and \texttt{KACC\_CXX}.

\subsection{Runtime environment variables}
\begin{description}
\item [KAAPI\_CPUSET]: if defined, and if the operating system supports it, then the variable specifies the set of cores to use in \kaapi program. If not defined, the operating system decides to schedule \kaapi threads onto the cores.
...
A parallel  SMPSs program running on a multicore is composed of several threads sharing the same address space. The threads execute tasks which may shared values in the address space, that we call thread shared memory.
