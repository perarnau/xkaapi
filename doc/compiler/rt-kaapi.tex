...
\end{center}
\vspace*{-2ex}
\caption{Example memory region declarations}
...
In the accumulate code of figure~\ref{fig:exaccu} FIXME recursively splits the computation until a grain of 2. Each created sub-tasks works on a disjoint parts of the initial input array. 

This example is simple: no temporary data are required at each level of recursion. This is not the case with the recursive computation of the Fibonacci number of figure~\ref{ex:fibonacci} page~\pageref{ex:fibonacci}.
As discussed in section~\ref{sec:optimalloca}, the user must mind the scope of data allocated during the computation.
%
%
%\begin{figure}[ht]
%\begin{center}
...
%%%%
\subsubsection{Example with recursive task creations}\label{sec:alloca}
...
To control the allocation scope of automatic variables allocated in the C stack of example in figure~\ref{fig:recaccumalloca}, the programmer has to use the directive \texttt{\#pragma data alloca} at line 11, otherwise variables \texttt{subresult1} and \texttt{subresult2} are undefined after the end of scope '\texttt{\}}' at line 17.
\begin{figure}[ht]
\begin{center}
\begin{minipage}[t]{0.9\linewidth}
%\scriptsize
...
specified to have the same scope as the created task using the directive \texttt{\#pragma kaapi alloca}.}
...
\item The detection of dependency between tasks uses a special memory address of the range declaration FIXME??? which is named the \textbf{referent}. This address is the address of the identifier in case of the D-dimensional format. And for 1D definition using the format \verb+[identifier .. identifier]+ this is the memory address given by the value of the first identifier (a pointer).
\end{enumerate}
