...
\end{itemize}
The \kaapi libraries and runtime are available on \url{http://kaapi.gforge.inria.fr} (tarball) or as debian package. \kaapi libraries and runtime is distributed with a CeCILL-C license\footnote{\url{http://www.cecill.info/index.en.html} for english version}:
\begin{center}
...
In the remaining, we only present the \kaapi programming model and we show examples using the \verb+kacc+ compiler only. The process of describing parallel program with \kaapi is:
\begin{itemize}
...
The parallel region begins at the next basic block (between \verb+{+ and \verb+}+) and it ends at the end of the basic block. It could also be defined before any instruction and it will end after the instruction.

The \texttt{\#pragma kaapi parallel} defines a region where concurrent execution may occur. The number of threads used to execute tasks inside parallel region may varies during the execution. 
...
#pragma kaapi task mode( array{ storage=Fortran; \
                                ...
                                [n][m] } )
\end{verbatim}
Where the \texttt{lda} must be a reference to a formal parameter name. The dimension must follow the subset of the C grammar for expression. 
C or Fortran keywords are used to specify the storage of the matrix (row major or column major).
...
)
{
  ...
  for (i = 0; i < block_count; ++i)
  {
    for (j = 0; j < block_count; ++j)
    {
      ...
      accum_block_hist(p, CONFIG_BLOCK_DIM, lda, hist, CONFIG_DATA_DIM);
    }
  }
}
