...
\RRetitle{The \kaapi's Application Programming Interface.
 \Huge{Part I: Data Flow Programming}}
%%
...
In this report, we present \kaapi's programming model. A \kaapi parallel program is a C or C++ sequential program with code annotation using \texttt{\#pragma} compiler directives that allow to create tasks. A specific source to source compiler translates \kaapi directives to runtime calls.
}
...
The \kaapi programming model comes from Athapascan~\cite{Athapascan98} which, itself, was inspired by Jade~\cite{Jade:1998} and Cilk~\cite{Cilk5:1998}. Because the \kaapi programming model and  StarSs~\cite{Badia:2009} are very close, we also accept the execution of StarSs programs. Descriptions of \kaapi may be found in following publications~\cite{Kaapi_2007,Traore_2008-deque_free, CCK_MCO08,KAAPI_TIC_2009}.

This technical report is organized as follows. Next section describes how to install \kaapi. Section~\ref{sec:helloworld} presents 
though simple examples how to program with \kaapi in C or C++. Section~\ref{sec:progmodel} describes the programming model and all the \texttt{\#pragma} annotations and the semantics of \kaapi program. Several complete examples are given in section~\ref{sec:exp}.
...
\kaapi is developed in the MOAIS INRIA project by Thierry Gautier, Fabien Le Mentec, Vincent Danjean and Christophe Laferrière in the early stage of the library.
...
\begin{itemize}
\item GNU-Linux on x86\_64 architectures,
...
\begin{itemize}
\item install the ROSE compiler framework,
\item compile the \kaapi library and the \kaapi compiler,
\item install the \kaapi library and the \kaapi compiler.
\end{itemize}
...
Moreover, the \kaapi compiler is build on top of the ROSE\footnote{\url{http://www.rosecompiler.org}} framework to develop source-to-source translators. In this framework, the C and C++ frontends are based on the EDG frontend. 
...
\subsubsection{\kaapi library and compiler installation}
...
options can be used to modify the default behavior. A list of the
...
\item \verb+--with-hwloc+\newline
To compile with hwloc (\url{http://www.open-mpi.org/projects/hwloc}) library to take advantage of the topological information of the computer.
...
The KaCC compiler is located in \texttt{<prefix>/bin}, it needs to be set in your \texttt{\$PATH} variable to avoid full path naming.
All programs compiled with KaCC are linked against libraries located in  \texttt{<prefix>/lib}. 
Executables linked with KaCC do not require to add this directory in your \verb+LD_LIBRARY_PATH+ under linux, and \verb+DYLD_LIBRARY_PATH+ under Mac OS X.
...
A presentation of the examples using the \kaapi library directly  (with C, C++ or STL) interface is out of the scope of this report.
...
\begin{itemize}
\item \kaapi has a task based parallel programming model: the user must indicate where tasks are.
\item A task is a function call without side effect: the user must annotate the code to specify what function is a task. 
\item The compiler inserts task creation code for each appropriate function call.
\item Dependencies between tasks are automatically computed by the runtime through data flow analysis between function calls: when a function is annotated as a task, the user must specify the memory access mode for each formal parameter. 
\end{itemize}
...
The example of figure~\ref{ex:helloworld} presents the creation of one task that prints "Hello world". The
...
The construction \texttt{\#pragma kaapi parallel} at line 11 starts the parallel region:
it begins at the next basic block (between \verb+{+ and \verb+}+) and it ends at the end of the basic block. 
...
\end{figure}
The task in the figure takes one parameter (a C string) and displays it to the screen. The clause \texttt{value(msg)} tells the compiler the effective parameter is considered as being passed by value. We will see in the following sections what it means.
...
The next example in figure~\ref{ex:helloworld2} creates 2 independent tasks that print 2 messages.
\begin{figure}[h]
\begin{center}
\begin{minipage}{0.99\linewidth}
\begin{footnotesize}
...
There are 3 ways for \kaapi to enforce an execution order:
\begin{itemize}
...
In \kaapi, any parameter that has to be shared between tasks must be a pointer. In the previous example, it is a C-string, \textit{i.e.} a pointer to a null terminated array of characters.


\subsection{Recursive computation: Fibonacci}
Figure~\ref{ex:fibonacci} presents a recursive computation of the $n$-th Fibonacci number\footnote{For simplification, $n$ was fixed to 30 in the code.} using the classical non optimal algorithm.
...
The main function creates two tasks: a first one to do the computation (line 29) and a second one to print the result (line 30).
Due to access modes, those two tasks have a true dependency. 
%Like the other examples, those tasks are created and executed inside a parallel region defined in the main function, at line FIXME

The \verb+fibonacci+ task (lines 3-16) recursively creates two tasks until the input parameter \texttt{n} gets smaller than \texttt{2}.
%%In \kaapi, tasks must be created into a parallel region: in \kaapi \textbf{each body of a task is an implicit parallel region}.

A task creation is a non blocking call, thus at line 13 a synchronization point is added (\texttt{\#pragma kaapi sync}):
\begin{itemize}
\item this directive enforces that all previously created tasks in the same function body\footnote{More formally, in the same activation frame as the function body execution.} be completed before proceeding with the next instruction, at line 14, to sum the sub results.
\item this directive does \textbf{not} wait for the completion of tasks created in the outter function calls.
\end{itemize}



\subsubsection{Optimization}\label{sec:optimalloca}
The code of figure~\ref{ex:fibonacci} has a main drawback: it requires a synchronization at each step of the recursion. Even if, in the general case of execution, this synchronization does not wait because tasks are generally executed sequentially, it adds an extra overhead. 

In this section, we present how to suppress this overhead by creating a task that will represent the \textit{continuation} of the computation.
The code is presented in figure~\ref{ex:fibonacci2}.
...
The advantage of the continuation passing style is to replace an explicit coarse grain synchronization (\texttt{\#pragma kaapi sync}) by an implicit data flow synchronization. It is done by creating the task \texttt{sum}.

Because the tasks created at lines 16, 17 and 18 are non blocking calls, the user must take care of the scope of the variables passed to the tasks. In the previous example, because \texttt{r1} and \texttt{r2} are allocated in the C stack of the function \texttt{fibonacci}, the user must specify at line 14 the directive  \texttt{\#pragma kaapi data alloca} to annotate explicitly those data: Then the compiler will allocate them so that their scope is sufficient for the execution of the task.
...
\newpage\section{\kaapi programming model}~\label{sec:progmodel}
...
The execution of a \kaapi program generates a sequence of tasks that access to data in a shared memory. From this sequence, the runtime is able to extract independent tasks in order to dispatch them to idle cores.
...
A \kaapi program begins by executing the C/C++ main entry point of the process. Then, everytime a call to an annotated task function is encountered, a task is created and the control flow proceeds its execution until it reaches a synchronization point.\\
 
A task is a function call: a function that should return no value except through the shared memory and the list of its effective parameters. It is of the responsibility of the programmer to annotate functions which calls become tasks, by using the \texttt{\#pragma kaapi task} directive. 
...
A task implements a sequential computation whose granularity is fixed by the user; it is created in program statements that correspond to calls to functions annotated as tasks thanks to the \texttt{\#pragma kaapi task} directive.
Tasks share data if they have access to the same memory region. A memory region is defined as a set of addresses in the process virtual address space. This set has the shape of a multi-dimensional array.

The user is responsible for indicating the mode each task uses to access to the memory: the main access modes are \textit{read}, \textit{write}, \textit{reduction} or \textit{exclusive}.\\

One of the main characteristics of the \kaapi programming model is the preservation of the sequential semantic of the program: the result of the parallel execution remains the same as the sequential execution of the program compiled without \kaapi.


%%%
\subsection{Task}
\kaapi is called a macro data flow language: The user is responsible for defining the granularity of the computation (task) as well as data shared between tasks. The runtime automatically schedules tasks in any order that respects the true data flow dependencies, \textit{i.e.} the data flow dependencies. All other dependencies are subject to be removed by renaming variables. 

This section presents how to annotate a function declaration in such a way that ALL calls to this function are translated to creations of tasks. This is called \textit{function task} annotation.
%The next section~\ref{sec:contextualtask} presents the annotation of a statement or of a block of statements in order to automatically convert it to a task creation. This is called \textit{anonymous task} annotation of block.
...
\item In case of library of functions where function declarations cannot be changed, the users must re-declare the functions with the \kaapi corresponding annotations.
\end{itemize}
...
The code of figure~\ref{fig:accummainkaapi} illustrates the annotation of two functions selected to be \textit{function tasks}:
\begin{itemize}
...
\end{center}
\vspace*{-2ex}
\caption{Examples of two declarations of function tasks}
...
\label{fig:exdecldef}
\end{figure}
In figure \ref{fig:exdecldef}, line 1 and line 2 correspond to the function task declaration. Line 3 through line 4 to the function task definition.

Figure~\ref{fig:exdecldef2} illustrates the case where the declaration and the definition are given by the same statement, i.e. this is a defining declaration of the function:
\begin{figure}[ht]
\begin{center}
\begin{minipage}[t]{0.9\linewidth}
\begin{footnotesize}
...
After the declaration of a function task, The \kaapi compiler replaces every call to the function by a task creation.
...
This implies that a variable written by a task can be read:
\begin{itemize}
...
%%TODO\subsubsection{Anonymous task}\label{sec:contextualtask}
...
\begin{itemize}
\item A statement,
\item A basic block of statements defined between braces ('\texttt{\{}' and '\texttt{\}}').
\end{itemize}
Every tasks created inside a parallel region may be executed concurrently:
...
Before the control flow enters a parallel region, the execution is driven by the main thread.
...
\begin{enumerate} 
\item \kaapi creates \texttt{KAAPI\_CPUCOUNT} threads on the cores given by the variable \texttt{KAAPI\_CPUSET}.
\item Else, if \texttt{KAAPI\_CPUCOUNT} is undefined, \kaapi creates one thread mapped on each core defined in \texttt{KAAPI\_CPUSET}.
\item Else, if \texttt{KAAPI\_CPUSET} is undefined, \kaapi creates \texttt{KAAPI\_CPUCOUNT} threads scheduled by the operating system.
\item Else, \kaapi creates one thread per physical core of the machine.
\end{enumerate}
...
\begin{enumerate}
\item an implicit synchronization point is inserted at the end of every parallel region; 
...
Several access modes\footnote{These access modes have either a direct correspondence in SMPSs/StarSs, called direction as in CORBA. Same access modes exist in Quark, the PLASMA abstract layer for task creations.} have been defined based on our original work in Athapascan~\cite{Athapascan98}: \textbf{read}, \textbf{write}, \textbf{readwrite} or \textbf{exclusive} and \textbf{cumulative write} (also called \textbf{reduction}). Each of those access modes limits the kind of operation a task can perform on a memory region. For instance, a task that declares an read access mode for one of its effective parameters cannot write.
...
A task formal parameter that is not annotated by an access mode is considered as a compilation error.
...
The \texttt{<list declaration>} defines a list of memory regions need by the task. 
The memory region is described by\
\begin{itemize} 
\item A memory address given by a variable.
\item A shape that describes a multidimensional array.
\end{itemize} 
In listing of figure~\ref{fig:exmemregion1}, line 1 declares 3 memory regions:
\begin{enumerate}
\item \texttt{n}: this is the memory region that begins at the address of the variable \texttt{n} with a size of \texttt{sizeof(int)} byte long.
\item \texttt{array[n]}: this a 1-dimensional array that starts at address \texttt{array} and finishes at address \texttt{array+n}.
\item \texttt{result}: its memory region corresponds to the storage of the variable of type \texttt{int} pointed by the value of \texttt{result}.
\end{enumerate}
...
void accumulate( int n, int* array, int* result );
\end{verbatim}
The dimension of array must follow the subset of the C grammar for expression (called \textit{additive expression} in C grammar, see appendix~\ref{sec:grammar}). 
At runtime, the base address is the value of a pointer type formal parameter.

The second form is to specify an interval between two addresses (as used in most of the STL algorithms). The syntax for an interval is \texttt{[a:b]}, where \texttt{a} and \texttt{b} are identifiers of formal parameters.
...
\paragraph{2-D array.}~\\
2-D arrays appear de facto in linear algebra subroutines. 
To split data structures for parallel sub-computations, it is necessary to handle subsets of 2-D arrays.

\kaapi  follows the design of BLAS or LAPACK libraries: the matrix is assumed to be allocated into a contiguous array and the user must specify the \textit{leading dimension} (\textit{ld} in figure~\ref{fig:matrix}), which is the size\footnote{The size is expressed with respect to the \textit{element type}.}  to pass from one row to the next one with the C storage (from one column to the next column with Fortran convention).
...
\begin{itemize}
\item The \textit{leading dimension}. % (\texttt{ld} above) must be an identifier of a formal parameter. 
...
\item The storage attribute: C or Fortran (or F) keywords are used to specify the storage of the matrix. \texttt{storage=C} for row major C storage. And \texttt{storage=Fortran} for Fortran column major.
If not specified, the default value is the \texttt{C}.
\item The dimension must follow the subset of the C grammar for expression. 
\end{itemize}
There is no order to specify attributes.
All those rules are defined in the grammar in the appendix~\ref{sec:grammar}.

%\paragraph{Examples.}~\\
%The following examples show different declaration matrix memory regions.
%\begin{itemize}
%\item A contiguous memory region that represent a C storage matrix \verb+A+ of dimension \verb+n+ rows by \verb+m+ columns:
%\begin{verbatim}
%A[n][m]
%\end{verbatim}
%Note that this definition is equivalent to the memory region:
%\begin{verbatim}
%A[n*m]
%\end{verbatim}
%\item A C storage matrix \verb+A+ of dimension \verb+n+ rows by \verb+m+ columns with leading dimension \verb+ldA+:
%\begin{verbatim}
%A{ld=ldA, [n][m]}
%\end{verbatim}
%\item A Fortran storage matrix \verb+A+ of dimension \verb+n+ rows by \verb+m+ columns with leading dimension \verb+ldA+:
%\begin{verbatim}
%A{ld=ldA, storage=F, [n][m]}
%\end{verbatim}
%\end{itemize}

\subsubsection{Pass-by-value rule}
Any function parameter passed by value must have a value access mode.
In this case, the effective parameter is copied at the task creation time and any modification remains local to the function.

Any C or C++ type can be defined to be passed by value.
The syntax is the following:
\begin{verbatim}
#pragma kaapi task value( <list declaration> )
\end{verbatim}
The rule \texttt{<list declaration>} is given in the appendix~\ref{sec:grammar} and corresponds to a list of memory regions separated by '\verb+,+'.
...
The access mode name \texttt{mode} of the clause is given by table~\ref{table:accessmode} page~\pageref{table:accessmode}:
\begin{description}
\item [write:] the task can write to any location of the declared memory region. Any read instruction is guarantied to return the value produced by the task. A read instruction to a location not written by the task returns an undefined value.
\item [read:] the task that reads a location in the memory region declared \texttt{read} accesses the value produced by the previous task in the lexicographic order of tasks in the program, that required a write or read-write or cumulative access. If no such task existed, then the value is the value given during initialization.

The task cannot write location of the memory region.
\item [readwrite:] the task can read or write to any location of the memory region. As for \texttt{read} access mode, a read instruction returns the value previously written by the task or the value produced by a previous task or at initialization.
\item [reduction:] this is the subject of the next section.
\end{description}

%%%%
\subsubsection{Cumulative access mode}\label{sec:cw}
In \kaapi, it is possible to reduce variable with associative operator. The commutativity is not required.
A task that requires an accumulative semantic with respect to one of its formal parameters, must declare it with the \texttt{reduction} clause in the \texttt{\#pragma kaapi task}. 
...
Two tasks having a reduction access mode to the same memory region are concurrent. The two tasks created at line 12 and 13 in listing of figure~\ref{fig:exaccu} illustrate this case.
\begin{figure}[ht]
\begin{center}
\begin{minipage}[t]{0.99\linewidth}
\begin{footnotesize}
\begin{lstlisting}[frame=tb]
#pragma kaapi task read([begin:end]) reduction(+: result)
void accumulate (double* begin, double* end, int *result) 
{
  ...
  if (size < THRESHOLD)
  {
    ...
    size_t med = size/2;
    accumulate( begin, begin+med, result);
    accumulate( begin + med, end, result);
  }
\end{lstlisting}
\end{footnotesize}
\end{minipage}
...
\label{fig:exaccu}
\end{figure}
The reduction operator defined in the \texttt{reduction} clause is used to merge the result produced by a task theft.
In listing~\ref{fig:exaccu}, if one thread performs all the tasks, then only the operator \texttt{+=} at line 8 is used.
If one of the task created at line 12 or 13 is theft, then the thief thread performs the task and reduce results on a new variable. This new variable is initialized as described below.

\paragraph{Predefined reduction operators.}~\\
For all the scalar types of the C or C++ language, \kaapi predefines a list of known operators (table \ref{table:opcumul}).
For each operator, the neutral element corresponds to the neutral element of the mathematical operator. The list which is presented in table~\ref{table:opcumul} follows the same predefined reduction operators as in OpenMP-3.0.
\begin{table}[h]
\begin{center}
\small \footnotesize
\begin{tabular}{c|c|c}
...
\paragraph{User defined reduction operator.}~\\
The user has the ability to define its own reduction operator using \texttt{\#pragma kaapi declare reduction}:
\begin{description}
\item [\texttt{\#pragma kaapi declare reduction}]~\\
\verb!(reduction-identifier : function-name)!\\
\verb![ identity( function-name) ]!\\
The \verb!function-name! must correspond to the name of a C/C++ function that must satisfy one of the following signature:
...
\begin{description}
\item [Read after Write.] Also called true dependency. A task is reading a memory region produced by another task.
\item [Write after Write.] False dependency. Such dependency is suppressed using variable renaming: the two tasks can be executed concurrently if they work on distinct memory region, thus the runtime may allocate distinct memory regions.
\item [Write after Read.] This false dependency is an anti-dependency. Variable renaming suppress them.
\end{description}

Note that the \kaapi runtime used a lazy approach to remove false dependencies: if it is necessary to concurrently execute two tasks with false dependencies, then the runtime renames the memory region. Else, the execution is sequential, using the task creation order.
...
For instance, the synchronization point at line 13 of the recursive computation of the Fibonacci number in listing~\ref{ex:fibonacci} page~\pageref{ex:fibonacci} enforces completion of the two tasks created at lines 11 and 12 only.


\subsubsection{\#pragma kaapi sync(<list\_declaration>)}
...
#pragma kaapi sync(subresult1)
  printf("Subresult is: %i\n", subresult1 );
...
\end{minipage}
\end{center}
\caption{Section of variable to enforce synchronization.}\label{ex:sync}
\end{figure}

This directive is a finer version of \texttt{\#pragma kaapi sync}.

%%%%
%%%
\subsection{Recursive function call}\label{sec:recursive}

\kaapi allows for recursive task creation. Several examples have been presented in section~\ref{sec:helloworld}.
%This is illustrated in figure~\ref{fig:exaccu} (page~\pageref{fig:exaccu}).
The accumulate code of figure~\ref{fig:exaccu} (page~\pageref{fig:exaccu}) recursively splits the computation until a grain of 2. Created sub-tasks work on disjoint part of the initial input array. 

This example is simple: no temporary data is required at each level of recursion. This is not the case with the recursive computation of the Fibonacci number in figure~\ref{ex:fibonacci} page~\pageref{ex:fibonacci}.
As discussed in section~\ref{sec:optimalloca}, the user should ensure enough scope of data allocated during the computation.
%
%
%\begin{figure}[ht]
%\begin{center}
...
For instance, because a task creation is a non-blocking call, the example of figure~\ref{ex:caplante} is incorrect: the automatic variable \texttt{r} will be suppressed before the execution of task \texttt{g}.
\begin{figure}[ht]
\begin{center}
\begin{minipage}{0.99\linewidth}
\begin{footnotesize}
...
if (...)
{
  int r; /* store sub result */
  g(&r, input_values); /* create task*/
}
\end{lstlisting}
\end{footnotesize}
\end{minipage}
...
\begin{itemize}
\item Add a synchronization point between line 4 and 5. It enforces the execution to wait for the completion of \texttt{g} (the 2 solutions at the top of figure~\ref{ex:caplanteplus}).
\item Allocate the data in the same scope as the tasks. In that case, the data is allocated with a scope wide enough to execute the task \texttt{g} (bottom solution in figure~\ref{ex:caplanteplus}).
\end{itemize}
...
\end{center}
\vspace*{-2ex}
\caption{Solutions for incorrect code of figure~\ref{ex:caplante}}
...
The \verb!<list of variables>! describes a list of automatic variables to be used in task creations.
\end{description}

The \kaapi name of this directive is analog to the \texttt{alloca} function of the stdlib C library.  With sequential C or C++ program, the developper may allocate data in the stack using the \texttt{alloca} function. In that case, the allocation scope is the same as the activation frame where the call to \texttt{alloca} occured.
With \texttt{\#pragma kaapi alloca}, the user can allocate data within the same scope as the created tasks.

%%%%
\subsubsection{Synchronisation and recursive tasks}\label{sec:alloca}
In a recursive computation, the \texttt{\#pragma kaapi sync} directive inside a task only synchronizes the child tasks spawned by the task instance. Tasks spawned by an ancestor tasks are note synchronized\footnote{Cilk+ has the same semantic for its sync keyword.}.

%%%%%
%\subsubsection{Example with recursive tasks}\label{sec:alloca}
%
...
%
%To control the allocation scope of automatic variables allocated into the C stack of example in figure~\ref{fig:recaccumalloca}, the programmer has to use the directive \texttt{\#pragma data alloca}  at line 11, else variables \texttt{subresult1} and \texttt{subresult2} are undefined after the end of scope '\texttt{\}}' at line 17.
%\begin{figure}[ht]
%\begin{center}
%\begin{minipage}[t]{0.9\linewidth}
%%\scriptsize
%\begin{footnotesize}
%\begin{lstlisting}[frame=tb]
%#pragma kaapi task read(result1, result2) write(result) 
%void sum (int* result, int * result1, int* result2) 
%{
%  *result = *result1 + *result2;
%}
%
%#pragma kaapi task read(n, array[n]) write(result) 
%void accumulate (int n, int* array, int *result) 
%{
%  if (n < 2) *result += array[0] + array[1];
%  else {
%#pragma kaapi data alloca(subresult1, subresult2);
%     int subresult1;
%     int subresult2;
%     accumulate (n/2, array, &subresult1);
%     accumulate (n-n/2, array+n/2, &subresult2);
...
%specify to have the same scope of the created task using directive \texttt{\#pragma kaapi alloca}.}
...
This example shows that one of the main difference with respect to the StartSs programming model is its ability to view sub-matrix of matrix without requiring to reallocate the user data structure.\\
...
#pragma kaapi task value(Order, Uplo, N, lda) \
                   readwrite(A{ld=lda; [N][N]})
int clapack_dpotrf(const enum ATLAS_ORDER Order, 
                   const enum ATLAS_UPLO Uplo,
                   const int N, double *A, const int lda);


#pragma kaapi task value(Order,Side,Uplo,TransA,Diag,M,N)\
                   value(alpha,lda,ldb) \
                   read(A{ld=lda; [N][N]}) \
...
#pragma kaapi task value(Order,TransA,TransB,M,N,K)\
                   value(alpha,lda,ldb,beta,ldc) \
...
void main( int argc, char** argv )
{
  ...
  {
    printf("Cannot allocate matrice A, errno: %i\n", errno);
    return;
  }
...
{
  unsigned int x, y;
  for (y = 0; y < bdim; ++y, block += lda)
    for (x = 0; x < bdim; ++x, ++block)
      ++hist[*block];
}
  ...
  for (i = 0; i < block_count; ++i)
  {
    for (j = 0; j < block_count; ++j)
    {
      ...
      const unsigned int* const p =
      	block + i * block_size + j * CONFIG_BLOCK_DIM;
      compute_block_hist
...
static unsigned int* gen_data(unsigned int dim)
{
  ...
  unsigned int* const block = malloc(size * sizeof(unsigned int));

  /* arrange so that each value appears dim times */
  unsigned int i;
  for (i = 0; i < size; ++i) block[i] = i & (dim - 1);
  return block;
}
...
int main(int ac, char** av)
{
  ...
  unsigned int* const block = gen_data(CONFIG_DATA_DIM);
  int err;

  if (block == NULL) return -1;

#pragma kaapi parallel
  compute_hist(block, CONFIG_DATA_DIM, hist);
...
The support of C and C++ relies on the version of the EDG frontend used internally by the ROSE framework. C is seems to entirely supported\footnote{See ROSE/EDG documentation.}. C++ program is not well recognized by the EDG frontend\footnote{We have problems to recognized complex C++ program that mix template and specialization.}.

For recognized C++ program, we extend the capacity to declare tasks for method invocations. The object pointed by \texttt{this} is implicitly passed with read-write mode, except for \texttt{const} method.
...
If the reduction operator for \verb+reduction+ access mode is left unspecified, they the default operator is \verb'+' for all the scalar type (integral value and floating point value). 
For other instances of type or class that do not have the \verb'+' operator defined, this is an compiler error. For scalar values (integral value and floating value), the default identity operator is to set the value '0' which is the neutral element with respect to the associative default \verb'+' operator. For other type or class which have a defined \verb'+' operator, then the neutral is assumed to be set by calling the default constructor.

\subsubsection{Note on range declaration}

In the range declaration rule, the programmer has the opportunity to define the set of memory address that the task will access. 
...
The compiler directive \verb+#pragma kaapi declare <reduction_declaration>+ follows the next grammar. This grammar may be extended to match the OpenMP directive to declare reduction operators using pseudo variables.
\begin{verbatim}
...
\end{verbatim}
The role of the identity declaration is to specify the initialization of a variable with the value that corresponds to the neutral element with respect to the reduction operator. For instance: \texttt{0} for operator \texttt{+} over scalar type.
For user defined types, this function should be defined. In C++, if the identity function is not defined, the C++ empty constructor is used to initialize the value. In C, the default identity function sets to zero the data structure. 
...
\end{verbatim}
This directive must be defined in a basic block before the definition of automatic variables.
This is a compiler error to pass a non-automatic name (\textit{e.g.} global variable) or to declare the pragma after the definition of the variables.
...
\begin{enumerate}
\item Each \kaapi directive that begins with \texttt{\#pragma kaapi} is interpreted and the code is rewritten to call functions of the \kaapi library. Invalid use of \kaapi directive and some warning are outputed.
...
\begin{description}
\item [-\,-keep]: keep the intermediate files. For each file, one intermediate file is generated. It is prefixed with \texttt{kaapi-} and is the translation of the file with replacement of function calls by task creations. 
\end{description}


\subsection{Backend compiler selection}
The backend compiler is the default C or C++ compiler defined during the installation step (see options with \texttt{configure --help}). It can be changed by setting the environment variables \texttt{CC} and \texttt{CXX}.

\subsection{Runtime environment variables}
\begin{description}
\item [KAAPI\_CPUSET]: if defined, and if the operating system supports it, then this variable specifies the set of cores to use in \kaapi programs. If not defined, the operating system decides to schedule \kaapi threads onto cores.
...
A SMPSs parallel  program running on a multicore is composed of several threads sharing the same address space. The threads execute tasks which may share values in the address space, that we call thread shared memory.
A task is a function call: a function and the list of its effective parameters. 
In  SMPSs~\cite{smpss-manual} the definition of task is based on the annotation of the C function as in \kaapi. Figure~\ref{fig:accumulatesmpss} illustrates the methodology. The original sequential code \verb+accumulate+ is annotated to be a task using SMPSs annotation (\verb+#pragma css task+). Each formal parameter is described by the access the task performs: \verb+input+ means that the task requires it to be ready before execution; \verb+output+ means that the task will produce it. And \verb+inout+ means both. As depicted in the code, a formal parameter may be defined with a size in order to identify the memory region accessed by the task.
\begin{figure}[ht]
\begin{center}
\hrule\vspace*{1ex}
\begin{minipage}[t]{0.9\linewidth}
...
All tasks are asynchronous as in \kaapi.
...
The \kaapi compiler parses and executes correctly all the SMPSs directives without any change in the source code. 

\subsection{Incompatibility}
Moreover, due to its ability to define memory region of submatrices, \kaapi can be used where SMPSs requires source code modification (for instance in all  inplace SMPSs factorization algorithms -LU, Cholesky, QR, ...-) where data (submatrix) must be reallocated into contiguous memory region.

The semantic of reduction variable in \kaapi allows to avoid locking mechanisms at the expense of a memory copy.
