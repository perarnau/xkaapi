...
\RRresume{Ce rapport d\'ecrit l'interface de programmation C pour \kaapi}

\RRmotcle{calcul parall\`ele, flot de donn\'ees, \kaapi, C}
\RRkeyword{parallel computing, data flow, \kaapi, C}
\RRprojets{MOAIS}
...
\subsection{Description}
\paragraph{}
A call to \verb+kaapic_spawn(n, entrypoint, e1, ..., en)+ creates a new computation task implemented by a call to a function \textit{entrypoint} with effective parameters \textit{e$_i$}.

The function \textit{entrypoint} as well as its effective parameters are passed in the optional parameter list of \textit{kaapic\_spawn}.
The format of the optional parameter list is:
\begin{description}
\item [entrypoint]: a C function 
\item [e$_1$]: the first effective parameter
\item [..]
\item [e$_{nargs}$]: the last effective parameter
\end{description}

\subsubsection{Format of the task entrypoint}
\begin{description}
\item [entrypoint]: the function with signature\\
...
\textit{entrypoint} is called with the user specified arguments, there is no argument added by \kaapi:\\
\begin{small}
\lstset{language=C}
\begin{lstlisting}[frame=tb]
entrypoint(e0, e1, ..., )
