...
%\thanksref{sfn}
  \and
Jean-Louis Roch%\thanks{Footnote for second author}%
 % r\'ef\'erence \`a la note partag\'ee
%\thanksref{sfn}
}
%%
%% Ceci apparait sur chaque page paire.
\authorhead{Le Mentec \& Gautier \& Danjean \& Roch}
%%
\RRtitle{The \kaapi's programming model and user's manual}
%% English title
\RRetitle{The \kaapi's programming model \\and \\User's manual for Adaptive Loop}
%%
...
\RRabstract{
In this report, we present the \kaapi's programming model about parallel adaptive loop. 
This report is the extension of the report "The \kaapi's programming model and user's manual" that presents the \kaapi's task model.
A parallel \kaapi program is a C or C++ sequential program with code annotation using \texttt{\#pragma} compiler directive. Then a specific source to source compiler translates \kaapi directives to call to the runtime.
}
...
%%%
\newpage
\section{Preface}\label{sec:userinstall}
...
In this report, only the programming model for \textbf{adaptive loop} based of code annotation with \texttt{pragma} compiler directive is presented. In this report we assume that the reader already knows  the \texttt{pragma} directives are presented in the first report~\cite{xkaapi:report1}, as well as the installation process of the \kaapi library and its compiler.
...
In the remaining, we only present the \kaapi programming model about parallel adaptive loop and we show examples using the \verb+kacc+ compiler only. The process of describing parallel program with \kaapi is:
\begin{itemize}
\item \kaapi is task based parallel programming model: the user should indicate where is task.
\item \kaapi is data parallel based parallel programming model: the user should indicate what loop must be parallelized.
\item A task is a function call without side effect: the user must annotate the code to specify what function is a task. 
\item The compiler will insert task creation code for each appropriate function call, or replace loop iteration with parallel loop iteration.
\item Dependencies between tasks are automatically computed by the runtime through data flow analysis between function calls: in the same time a function is annotated as task, the user must specify the access mode to the memory made for each formal parameters. 
\end{itemize}
...
The example of figure~\ref{ex:foreach} presents the declaration of a parallel loop that display "Hello world <i>", where \texttt{i} is the loop index.
\kaapi \verb+kacc+ compiler is based on pragma annotation, as for OpenMP, to describe the parallelism.

First it is necessary to declare a parallel loop where iteration may be executed concurrently.
Outside parallel loop the execution is driven only by the current thread. 
This is indicated by \texttt{\#pragma kaapi parallel loop} at line 8. 

The \texttt{\#pragma kaapi parallel loop} defines a region where concurrent execution may occur. The number of threads used to execute tasks inside parallel region may varies during the execution. 
...
\end{minipage}
\end{center}
\caption{hello\_world\_loop.c example}\label{ex:foreach}
\end{figure}

\subsubsection{Invoking the compiler}
To compile the previous example, simply type:
\begin{verbatim}
> kacc -o hello_world_loop hello_world_loop.c
\end{verbatim}
Then, run it with:
\begin{verbatim}
> ./hello_world_loop
...
The next example in figure~\ref{ex:foreach} apply an operator to each element of an array.
\begin{figure}[h]
\begin{center}
\begin{minipage}{0.9\linewidth}
\begin{small}
...
\end{minipage}
\end{center}
\caption{hello\_world2.c example}\label{ex:foreach}
\end{figure}
...
Note that at the end of a parallel region there is an implicit synchronization point: to enforce order in the previous example, the user may also have created tasks inside 2 parallel regions. The code of figure~\ref{ex:helloworld2} would become:
\begin{verbatim}
...
The following grammar describes the rules to define parallel loop
parameter in the \verb+#pragma kaapi parallel <parallelloop_declaration>+ compiler directive.
\begin{verbatim}
      parallelloop_declaration :=
          'loop'
        | 'loop' scheduling_directive
