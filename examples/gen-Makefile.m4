...
[PKGCONFIG_]AM_NAME([VAR])[_CFLAGS]=$(shell $(PKG_CONFIG) --cflags VAR)
[PKGCONFIG_]AM_NAME([VAR])[_LIBS]=$(shell $(PKG_CONFIG) --libs VAR)
])])dnl
dnl -----------------------------------------------------------
define([C_OBJS],
...
dnl -----------------------------------------------------------
define([PROG_RULES],[dnl
PROG_RULES_VAR(AM_NAME([$1]))
###################
# $1
...
[foreach_w([PROG],PROGS,[_CLEANUP_SOURCES([AM_GET([PROG],[SOURCES])])])])dnl
dnl -----------------------------------------------------------
define([Makefile],[dnl
# Autogenerated example Makefile
# This Makefile needed xkaapi to be already installed system-wide
# (ie pkg-config invocations must work)

################################################################
# Local configuration

# C compiler allowing C99
CC=
# C++ compiler
CXX=
...
CLEANUP_SOURCES[]dnl
PKGCONFIG_VARS([foreach_w([PROG],PROGS,[AM_GET(PROG,[PKGLIBS]) ])])
