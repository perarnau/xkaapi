...
void transform ( kaapi_steal_context_t* stealcontext, InputIterator begin, InputIterator end, OutputIterator to_fill, UnaryOperator op, int grain );
...
public:
  ...
  TransformStruct(
    ...
    UnaryOperator  op,
    int            grain
  ) : _sc(sc), _ibeg(ibeg), _iend(iend), _obeg(obeg), _op(op), _grain(grain) 
  {}
...
protected:  
  ...
  */
  static void splitter( kaapi_steal_context_t* stealcontext, int count, kaapi_steal_request_t** request, 
                        InputIterator ibeg, InputIterator& iend, OutputIterator obeg, UnaryOperator op, int grain
                      )
  {
    int i = 0;
...
void TransformStruct<InputIterator,OutputIterator,UnaryOperator>::doit()
{
  ...
  while (_iend != _ibeg)
  {
    ...
    */
    kaapi_stealpoint( _sc, splitter, _ibeg, _iend, _obeg, _op, unit_size );
...
void transform ( kaapi_steal_context_t* stealcontext, InputIterator begin, InputIterator end, OutputIterator to_fill, UnaryOperator op, int grain )
{
  ...
  TransformStruct<InputIterator, OutputIterator, UnaryOperator> work( stealcontext, begin, end, to_fill, op, grain);
  work.doit();
}
#endif
