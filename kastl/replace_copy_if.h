 ...
 *
 */
#ifndef _CKAAPI_REPLACE_COPY_IF_H
#define _CKAAPI_REPLACE_COPY_IF_H
#include "kaapi_adapt.h"
#include <algorithm>


template<class RandomAccessIterator1, class RandomAccessIterator2, class Predicate, class T>
void replace_copy_if ( kaapi_steal_context_t* stealcontext, RandomAccessIterator1 begin, RandomAccessIterator1 end, 
            RandomAccessIterator2 res, Predicate pred, const T& new_value);
...
public:
  ...
  Replace_Copy_If_Struct(
    ...
    Predicate pred,
    const T& new_value
  ) : _sc(sc), _ibeg(ibeg), _iend(iend), _obeg(obeg), _pred(pred), _new_value(new_value) 
  {}
...
protected:  
  ...
  {
    Replace_Copy_If_Struct<RandomAccessIterator1, RandomAccessIterator2, Predicate, T>* w = (Replace_Copy_If_Struct<RandomAccessIterator1, RandomAccessIterator2, Predicate, T>*)data;
    w->_sc = sc;
    w->doit();
  }
  ...
  */
  static void splitter( kaapi_steal_context_t* stealcontext, int count, kaapi_steal_request_t** request, 
                        ...
                        RandomAccessIterator2 obeg, Predicate pred, const T& new_value
                      )
  {
    int i = 0;
    ...
    Replace_Copy_If_Struct<RandomAccessIterator1, RandomAccessIterator2, Predicate, T>* output_work =0;
    ...
    while (count >0)
    {
      if (request[i] !=0)
      {
        if (kaapi_steal_context_alloc_result( stealcontext, 
                                              request[i], 
                                              (void**)&output_work, 
                                              sizeof(Replace_Copy_If_Struct<RandomAccessIterator1, 
                                              RandomAccessIterator2, Predicate, T>) 
                                            ) ==0)
        {
...
*/
template<class RandomAccessIterator1, class RandomAccessIterator2, class Predicate, class T>
void Replace_Copy_If_Struct<RandomAccessIterator1, RandomAccessIterator2, Predicate, T>::doit()
{
  ...
  while (_iend != _ibeg)
  {
    ...
    */
    kaapi_stealpoint( _sc, splitter, _ibeg, _iend, _obeg, _pred, _new_value);
         ...
         RandomAccessIterator2 res, Predicate pred, const T& new_value)
{
  ...
  Replace_Copy_If_Struct<RandomAccessIterator1, RandomAccessIterator2, Predicate, T> work( stealcontext, begin, end, 
            res, pred, new_value);
  work.doit();
