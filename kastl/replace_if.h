 ...
 *
 */
#ifndef _CKAAPI_REPLACE_IF_H
#define _CKAAPI_REPLACE_IF_H
#include "kaapi_adapt.h"
#include <algorithm>


template<class RandomAccessIterator, class Predicate, class T>
void replace_if ( kaapi_steal_context_t* stealcontext, RandomAccessIterator begin, RandomAccessIterator end, 
...
public:
  ...
  Replace_If_Struct(
    ...
    Predicate pred,
    const T& new_value
  ) : _sc(sc), _ibeg(ibeg), _iend(iend), _pred(pred), _new_value(new_value) 
  {}
...
protected:  
  ...
  {
    Replace_If_Struct<RandomAccessIterator, Predicate, T>* w = (Replace_If_Struct<RandomAccessIterator, 
              Predicate, T>*)data;
    w->_sc = sc;
    w->doit();
  }
  ...
  */
                      ...
                      )
  {
    ...
    Replace_If_Struct<RandomAccessIterator, Predicate, T>* output_work =0;
    ...
    while (count >0)
    {
      if (request[i] !=0)
      {
        if (kaapi_steal_context_alloc_result( stealcontext, 
                                              request[i], 
                                              (void**)&output_work, 
                                              sizeof(Replace_If_Struct<RandomAccessIterator, Predicate, T>) 
                                            ) ==0)
        {
...
*/
template<class RandomAccessIterator, class Predicate, class T>
void Replace_If_Struct<RandomAccessIterator, Predicate, T>::doit()
{
  ...
  while (_iend != _ibeg)
  {
    ...
    */
    kaapi_stealpoint( _sc, splitter, _ibeg, _iend, _pred, _new_value);
                ...
                Predicate pred, const T& new_value)
{
  ...
  Replace_If_Struct<RandomAccessIterator, Predicate, T> work( stealcontext, begin, end, pred, new_value);
  work.doit();
