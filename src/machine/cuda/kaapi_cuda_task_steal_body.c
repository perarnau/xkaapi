...
#if 0
{
  ...
  for (i = 0; i < count_params; i++) {
    ...
    version =
	kaapi_version_findinsert(&islocal, thread, tasklist, access.data);
    if (version->last_mode == KAAPI_ACCESS_MODE_VOID) {
      kaapi_memory_view_t view =
	  kaapi_format_get_view_param(task_fmt, i, task->sp);
      kaapi_version_add_initialaccess(version, tasklist, m, access.data,
				      &view);
      islocal = 1;
    }
    handle =
